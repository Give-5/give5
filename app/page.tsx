...
export default function EntryPage() {
  ...
  console.log('[Entry] Rendering - loading:', loading, 'user:', user?.email || 'No user')

  // Redirect to home if already authenticated
  useEffect(() => {
    console.log('[Entry] Effect - loading:', loading, 'user:', user?.email || 'No user')
    if (!loading && user) {
      console.log('[Entry] Redirecting to home')
      router.push('/home')
    }
  }, [user, loading, router])
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-give5-light-bg flex items-center justify-center">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-8 py-12">
        ...
        <div className="w-full max-w-sm space-y-4">
          ...
          <button 
            ...
            className="w-full bg-white border border-gray-300 text-gray-700 py-3 px-6 rounded-full text-base font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
          >
